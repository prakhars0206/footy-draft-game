...
# so FifaDataLoader lifts older editions onto the modern scale — a FLAT +2 (FIFA ≤15) / +1 (FIFA 16) / 0 (17+),
# then capped at 95 (ERA_CAP). Flat keeps within-edition ranking order intact; the 95 cap only pulls a +2
# edition's raw-94 (prime Messi → 95) down from 96, staying one above the modern 94. DISPLAYED FIFA 07–16 ratings
# are therefore era-adjusted, not raw sofifa. (Analysis behind the choice: analysis/tier-distribution.md +
# analysis/flat-capped-scheme.md, generated by TierReport / CappedSchemeReport.)
